...
//TODO rename program to driver?
  ...
  while (feof(config_file_pointer)==0) //until the end of file
  {
    ...
    if (*line_buffer == 'V') //finds the first (and only) V to skip initial line
    {
      ...
      //file path section
      //file path is listed next so cycle through the words: 'testfile2.mdf', 'File', and 'Path:'
    ...
    {
      fscanf(config_file_pointer, "%s", log_file_path); //jump to next word
      if (*log_file_path == 'F') //checks if next word is 'File'
      {
        fscanf(config_file_pointer, "%s", log_file_path); //skip 'File'
        fscanf(config_file_pointer, "%s", log_file_path); //skip 'Path:' and store next word
    ...
    else if (*line_buffer == 'A')
    {
      ...
      fscanf(config_file_pointer, "%s", available_memory); //jump to next word
    ...
    {
      //config_file_pointer = config_file_pointer;//reset backup array
      fscanf(config_file_pointer, "%s", processor_cycle_time); //jump to next word
